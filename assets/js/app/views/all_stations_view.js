AllStationsView = BaseView.extend({
    ...
    initialize: function()
	  ...
	  var fragmentsCollection = new StationsCollection();
	  this.collection.each(function(m, i){
	      if(i%50 == 49)
	        {
		    $(docFragment).append(self.templateFragment(fragmentsCollection.toJSON()));
		    self.m_docFragments.push(docFragment);
		    docFragment = document.createDocumentFragment();
		    fragmentsCollection = new StationsCollection();
		}
	      fragmentsCollection.add(m);
	  });
	  console.log("nb fragments built : ", this.m_docFragments.length );
    ...
    render: function()
	  ...
	  $(window).on("scroll",function(){
		 ...
		 &&!self.m_isFiltered && n < self.m_docFragments.length){
		  console.log("append fragment : ", n);
		  $(".list-group").append($(self.m_docFragments[n]));
		  n++;
